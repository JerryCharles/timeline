...
export default function TopicDetail() {
  ...
  const isChinesePath = params.lang === 'zh';

  useEffect(() => {
    const savedLanguage = Cookies.get('language');
    if (savedLanguage && savedLanguage !== params.lang) {
      const newPath = savedLanguage === 'zh' 
        ? `/zh/topic/${params.id}`
        : `/topic/${params.id}`;
      router.push(newPath);
    }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="relative h-64 md:h-96 mb-6 rounded-lg overflow-hidden">
          <Image
            src={topic.image}
            alt={isChinesePath ? topic.titleCN : topic.title}
            fill
        ...
        <p className="text-lg mb-8 text-gray-700 dark:text-gray-300">
          {isChinesePath ? topic.summaryCN : topic.summary}
        </p>
              ...
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-grow">
                  ...
                  <div className="prose dark:prose-invert max-w-none">
                    <ReactMarkdown>
                      {isChinesePath ? event.contentCN : event.content}
                    </ReactMarkdown>
                  </div>
                  {event.url && (
                    <a
                  ...
                  {(isChinesePath ? event.labelsCN : event.labels).length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {(isChinesePath ? event.labelsCN : event.labels).map((label, i) => (
                        <span
                          key={i}
